...
function fn_ConfirmIdSend() {
	...
	$.ajax({
		...
		type: 'post',
		url: 'http://localhost:8081/sendId.sm', //실행할 파일(url), 데이터를 전송할 곳
		data: { user_id: user_id }, 
		// datatype: ?? -> ??에 정의한 datatype을 담을 수 있음
		success: function(data) {
			
			let data1 = $.trim(data);	// data에 spacemusicfrontcontroller에서 PrinWriter로 적은 result가 담김
			if (data1 == 'true') {		// db 다녀와서 idcheckaction에서 비교한 결과가 data1에 담김
				alert("사용 가능한 아이디입니다.");
		...
		error: function(xhr, status, error) {
			// error 대응을 사용자 입장에서 어떻게 편리하게 할 것인지 생각하고 하는것을 면접관들이 좋아함.
			//오류 발생 시 처리
		}
